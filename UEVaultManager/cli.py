...
class UEVaultManagerCLI:
  ...
  def list_assets(self, args):
    ...
    games, dlc_list = self.core.get_inner_asset_list( )
    ...
    if args.ue_assets_only:
      ...
      if args.csv or args.tsv:
        ...
        writer.writerow(
          [
            # dans les infos
            'App name', 'App title', 'Asset_id'
                                     'Image', 'Url', 'UE Version', 'compatible Versions', 'Review', 'Vendeur', 'Description', 'Categorie', 'Prix',
            'uid', 'Date Creation', 'Date Updated',
            'Status'
  ...
  def list_files(self, args):
    ...
    if args.hashlist:
      ...
      _files = [dict(filename=fm.filename, sha_hash=fm.hash.hex(), install_tags=fm.install_tags, file_size=fm.file_size, flags=fm.flags) for fm in
                files]
  ...
  def info(self, args):
    ...
    if game:
      ...
      game_infos.append(
        InfoItem(
          'Cloud saves supported', 'cloud_saves_supported', game.supports_cloud_saves or game.supports_mac_cloud_saves, game.supports_cloud_saves or
                                                            game.supports_mac_cloud_saves
        )
      )
      cs_dir = None
      ...
      if manifest.meta.prereq_ids:
        ...
        manifest_info.append(
          InfoItem(
            ...
            dict(ids=manifest.meta.prereq_ids, name=manifest.meta.prereq_name, path=manifest.meta.prereq_path, args=manifest.meta.prereq_args)
          )
        )
      else:
...
def main():
  ...
  parser.add_argument(
    '-A', '--api-timeout', dest='api_timeout', action='store', type=float, default=10, metavar='<seconds>',
  ...
  auth_parser.add_argument(
    '--code', dest='auth_code', action='store', metavar='<authorization code>',
    help='Use specified authorization code instead of interactive authentication'
  )
  auth_parser.add_argument(
    '--token', dest='ex_token', action='store', metavar='<exchange token>', help='Use specified exchange token instead of interactive authentication'
  )
  ...
  list_files_parser.add_argument(
    '--platform', dest='platform', action='store', metavar='<Platform>', type=str, help='Platform (default: Mac on macOS, otherwise Windows)'
  )
  ...
  list_files_parser.add_argument('--install-tag', dest='install_tag', action='store', metavar='<tag>', type=str,
